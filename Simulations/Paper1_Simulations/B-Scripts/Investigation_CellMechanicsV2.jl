...
Aₙ = (l₀, l₁, p) -> p.k .* ((l₁ .- l₀)./p.a .+ log.(l₀) .- log.(l₁))
...
F₂ = (δ,p) -> p.k .* (ones(size(δ))./p.a .- (1 ./ δ)) # Nonlinear restoring force (Nonlinear springs)
...
function plotForceCompare(l, F_hookean, F_nonlinear, a_hookean, a_nonlinear)
    ...
    gaxmain = Axis(ga[1, 1], limits=(0, l[end], ymin, ymax), aspect=AxisAspect(1), 
